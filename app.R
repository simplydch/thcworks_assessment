...
id_reviews <- relevant_reviews[relevant_reviews$user_id == init_id, c("completed_at", "relationship")]
...
review_dates <- id_reviews[id_reviews[,"relationship"]==review_type_sel, "completed_at"]

review_date_sel <- id_reviews[1, "completed_at"]
...
selected_date <- reactiveVal(review_date_sel)
...
server <- function(input, output, session) {
  ...
  ### Individual level reactive data
  ...
  observeEvent(input$id_sel, {
    if (!is.null(input$id_sel) && input$id_sel != "") {
      ...
      review_dates <- review_date_sub[review_date_sub[,"relationship"]==review_date_sub$relationship[1], "completed_at"]

      updateSelectInput(session, "review_date_sel",
                        choices = review_dates,
                        selected = review_dates[1]
      ) # Reset selection
      
      selected_type(review_date_sub$relationship[1])
      selected_date(review_dates[1])
  ...
  observeEvent(input$review_type_sel, {
      ...
      if(time_series_click()){
        ...
        updateSelectInput(session, "review_date_sel",
                          choices = review_date_sub$completed_at,
                          selected = review_date_sub$completed_at[1]
        ) 
        # Reset selection
        selected_date(review_date_sub$completed_at[1])
  ...
  observeEvent(input$review_date_sel, {
    isolate(selected_date(input$review_date_sel))
  })
  ...
  observeEvent(input$time_series_click, {
    ind_data_subset <- isolate(ind_plot_data()[["ind_data_subset"]])
    ...
    if (nrow(new_point) > 0 && "completed_at" %in% colnames(new_point)) {
      ...
      review_date_sub <- unique(data_subset_all_reviews[data_subset_all_reviews$user_id == input$id_sel &
                                                          data_subset_all_reviews$relationship == new_point$relationship[1] ,  
                                                        c("relationship", "completed_at")])
      

      updateSelectInput(session, "review_date_sel",
                        choices = review_date_sub$completed_at,
                        selected = new_point$completed_at[1]
      ) # Reset selection
      
      selected_type(new_point$relationship[1])
      selected_date(new_point$completed_at[1])
  ...
  output$info_box_days <- renderText({
    data_subset <- ind_plot_data()[["data_subset"]]
    return(as.numeric(cur_date - as.Date(selected_date())))
  })
  ...
  output$behave_bar <- renderPlot({
    ...
    point_col <- if (data_subset$relationship[1] == "Self") dark_pop_col else pop_col
    ggplot(data = av_score_per_behave, aes(
  ...
  output$hover_info <- renderText({
    ...
    if (bar_index >= 1 && bar_index <= nrow(data_subset)) {
      value <- av_score_per_behave$value[bar_index]
      category <- av_score_per_behave$behaviour_title[bar_index]
      return(category)
    } else {
      return("Hover over a bar to see the scored behaviour.")
    }
